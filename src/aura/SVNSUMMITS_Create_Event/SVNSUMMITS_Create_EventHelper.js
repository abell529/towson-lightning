...
({
	...
	getGroups: function (component) {
		...
		action.setCallback(this, function (response) {
			...
			groupIdCmp.set('v.value', valuesTemp[0].key);
		});
	...
	getGroupPostAll: function (component) {
		...
		action.setCallback(this, function (response) {
			...
			if (component.isValid() && state === "SUCCESS") {
				var postToAll = response.getReturnValue();

				component.set("v.canPostToAll", postToAll);
				self.debug(component, 'Groups: Can post to ALL: ' + postToAll);
			}
		});
	...
	toggleSubmitButton: function (component) {
		...
		var groupIdCmp = component.find("filterByGroup");
		var group = groupIdCmp.get('v.value');
